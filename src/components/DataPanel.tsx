...
const DataPanel: React.FC<DataPanelProps> = ({ onSign }) => {
  ...
  useEffect(() => {
    window.addEventListener('message', (event) => {
      console.log('[BLAZE_MARKETING] New event:', event.data);
      if (event.data.message === 'SIGN_REQUEST') {
        console.log('[BLAZE_MARKETING] Received sign request');
        setTimeout(() => {
  ...
  const handleSign = async () => {
    ...
    window.postMessage({ message: 'SIGN_REQUEST' });
    ...
    <div className="p-6 space-y-6" id="data-panel">
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex p-0.5 bg-gray-800/50 rounded-lg border border-gray-700/50">
            <button
              ...
              className={`text-xs px-3 py-1.5 rounded-md transition-colors ${activeTemplate === 'transfer'
            ...
            <button
              ...
              className={`text-xs px-3 py-1.5 rounded-md transition-colors ${activeTemplate === 'message'
                ? 'bg-primary-legacy text-white font-medium'
                : 'text-gray-400 hover:text-white'
                }`}
            >
        ...
        <div className="flex flex-col space-y-3">
          ...
          <button
            id="sign-data-btn"
            className={`w-full py-3 px-4 rounded-lg font-medium flex items-center justify-center space-x-2 transition-all ${isSigningInProgress
